...
| [分享范围](docs/sharing-scope.md) | 分享边界和后续更新方法 |
...
本仓库仅分享 Codex 技能、通用 Quarto 规范及制作说明。具体研究正文、实验数据、私人素材、个人配置、渲染输出和来源仓库 Git 历史均不在同步范围内；公开规范已移除私人路径与外部私人符号文件依赖。

更新时在独立仓库按文件清单同步，检查内容与暂存差异后提交。详细边界见 [分享范围](docs/sharing-scope.md)。仓库未配置自动构建、部署或同步；实际使用时须在自己的出版项目内运行预览、渲染并检查输出。
